...
# actually, isn't the likelihood how well each combo of parameters fits
# the data, which is an X and y. so the likelihood is a distribution of 
# the number of ways that each combo of paramters would produce the data.
# but still not sure if we're producing the data -- the X and y -- or 
# producing the dv/y? i guess it has to be, what are the proba of diff
# x_coeff and intercepts possibilites given these X and y pairs/data?
# and we specify here the actual data for y with observed=y. and specify
# that the x data is observed too.
...
# via prior distributions." Hmmm, little confusing. because think that each
# observed is defined by actual data? But i guess the model is asking for that
# observed data, what's the likelihood of each value of a paramter. and that 
# gets multiplied by the prior for that value of the paramter. ahhhh. I don't know
# if this all holds up, but anyhow.
